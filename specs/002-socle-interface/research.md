...
| E-23 | **Quatre fichiers de police, trois au premier affichage** : Public Sans 400 et 500, Archivo 700, et IBM Plex Mono 500 pour les données seules. Public Sans 600, Archivo 500 et 600 ne sont plus servies ; toute graisse 600 devient 500 (Public Sans) ou 700 (Archivo), et la synthèse de graisse est coupée. L'année de l'en-tête, le téléphone de l'administrateur, le retard et la version s'écrivent en Public Sans tabulaire | P-10 a mesuré 86,8 Ko de polices au premier affichage (six fichiers) pour un plafond de 45 : les composants dessinés sur la planche emploient plus de graisses que R-07 n'en comptait. La consigne est de réduire, jamais de relever | R-07 disait déjà « Public Sans 400 et 500, Archivo 700 au premier affichage » : c'est désormais vrai. Premier affichage mesuré à 43,6 Ko. La hiérarchie typographique perd un cran (600) ; la mono garde son rôle de données (matricule, montant, reçu, heure d'enregistrement) |
| E-24 | Les pages rendues par le serveur sont compressées (brotli, sinon gzip) par un greffon Nitro, `web/server/plugins/compression.ts` | `nitro.compressPublicAssets` ne compresse que les fichiers statiques : le HTML partait à 35 Ko | Le HTML de l'appel part à environ 8 Ko |
| E-25 | `NuxtLink` précharge une page voisine à l'interaction, plus à la visibilité ; la favicon est une icône de 32 px | Le premier affichage transférait le code et le style d'« à propos » et l'icône de 192 px | `experimental.defaults.nuxtLink.prefetchOn` ; `web/scripts/icones.mjs` produit `32.png` |
| E-26 | Le serveur de développement de la page de style est lancé par `scripts/avec-serveur-dev.sh`, partagé par P-05 et l'étape e2e | Les tests de la page de style (US1) tournent dans l'étape e2e, pas seulement dans P-05 | `verifier.sh` et `p-05.sh` l'emploient ; le port vient de `NELO_WEB_PORT_DEV` |
| E-27 | **Le mot d'une pastille ocre s'écrit en couleur de texte** ; l'ocre porte la voix par le fond, le point, le filet pointillé et l'icône du ruban hors ligne | axe a mesuré 3,2:1 pour `--accent` sur `--accent-soft` et 3,37:1 sur `--bg` en thème clair, sous l'AA (4,5:1) que FR-085 exige ; la planche 13 écrivait le mot en ocre. `theme.css` ne se réécrit pas | La voix reste lisible sans la couleur seule, le mot passe AA ; le contour « Proposé, non validé » écrit son mot en texte atténué |
| E-28 | Le filet des champs et de la recherche est une ombre intérieure ; les liens du fil d'Ariane, les onglets et l'action d'une alerte ont une largeur minimale de 44 px et la hauteur du contexte tactile ; l'en-tête de la coquille se replie sur deux lignes quand la largeur manque | Les tests d'US8 ont mesuré un champ à 50 px en classe (52 exigés), des cibles de 28 et 39 px de large, et un débordement horizontal à 390 px zoomé à 125 % | Corrections dans les composants, sans toucher au thème ni aux mesures |
| E-29 | Le contexte tactile se déclare par `definePageMeta({ contexteTactile })`, la coquille le fournit (`fournirContexteTactile`) et pose `--cible` sur toute l'application, en-tête compris | Les cibles de l'en-tête et de la navigation doivent suivre le contexte de l'écran (52 px en classe), ce qu'un `provide` posé par l'écran, sous la coquille, ne pouvait pas atteindre | `useContexteTactile()` lit le contexte ; la correspondance vers `--cible` vit dans ce seul composable |
| E-30 | Le serveur de développement est préchauffé dans Chromium avant les tests qui le visent | Sous la charge de la vérification complète, la première hydratation de la page de style dépassait le délai par défaut | `web/tests/prechauffer.mjs`, appelé par `scripts/avec-serveur-dev.sh` |
