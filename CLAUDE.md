...
- si un changement est nécessaire, le proposer comme **diff explicite sur le fichier projet AVANT de
  continuer**, sans diverger localement.
...
**N'utilise jamais le tiret cadratin `—`**, nulle part : ni dans tes réponses, ni dans le code et ses
commentaires, ni dans les specs, la documentation, les messages de commit ou les chaînes d'interface.
C'est la signature d'un texte généré. Choisis à la place la ponctuation que le contexte appelle :
deux-points pour introduire, virgule pour une incise, parenthèses pour un aparté, point pour couper.
